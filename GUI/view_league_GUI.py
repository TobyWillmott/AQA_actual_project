...
class ViewLeague(tk.Frame):
    def __init__(self, parent, user_id, league_id):
        ...
        self.players_frame = VerticalScrolledFrame(self, width=380, height=345, bg="#E5E5E5",
                                                   highlightbackground="black", highlightthickness=3)
        self.selections_frame = VerticalScrolledFrame(self, width=200, height=700, bg="#E5E5E5",
                                                      highlightbackground="black", highlightthickness=3)
        ...
        self.positions = [tk.Label(self, text=f"{position + 1}", bg="#E5E5E5") for position in
    ...
    def place_widgets(self):
        self.selections_frame.place(x=300, y=40)
    ...
    def display_matches(self):
        ...
        for i in range(len(self.user_games)):
            ...
            self.gameweek[i].grid(row=2*i, column=0)
            self.user_teams[i].grid(row=2*i, column=1)
            self.versus_label[i].grid(row=2*i, column=2)
            self.opp_teams[i].grid(row=2*i, column=3)
            self.user_teams_score[i].grid(row=2*i+1, column=1)
            self.opp_teams_score[i].grid(row=2*i+1, column=3)
